"""Cluster-level split-LRT compound e-values via universal inference.

Replaces src/evalues.py's Beta-MOM-cohesion construction with the
likelihood-ratio e-value derived in notes/construction.md (§§2-3).

For each cluster c with member embeddings x_{c,1}, ..., x_{c,n_c}:
  1. Random 50/50 within-cluster split (A_c, B_c).
  2. vMF MLE (μ̂_c, κ̂_c) on A_c (Banerjee et al. 2005 + Newton).
  3. log E_c = Σ_{i ∈ B_c} [log p_vMF(x_i | μ̂_c, κ̂_c) - log q̂(x_i)],
     where q̂ is the movMF mixture fit by src/q_movmf.py.
  4. K=2 swap-and-average: also compute E_c with (B,A); average via
     Vovk-Wang arithmetic mean.

Validity (Theorem 1, construction.md):
  E[E_c | A_c, H_c] = 1 by universal inference, conditional on the split.
  Closed under averaging by Vovk-Wang 2021. e-BH on {E_c} controls FDR ≤ α
  (Wang-Ramdas 2022) under arbitrary cluster dependence.

Output:
  data/processed/cluster_evalues_lrt.parquet — cluster_id, n, log_e, e
"""
...
def log_vmf_norm(d: int, kappa: float) -> float:
    """log C_d(κ).  Robust to ive underflow at small-κ-large-d via small-κ asymptotic."""
    if kappa < 1e-12:
        return -log_unit_sphere_area(d)
    nu = d / 2 - 1
    ive_val = float(ive(nu, kappa))
    if ive_val > 0.0:
        # standard path: log I_ν(κ) = log ive + κ
        log_I = float(np.log(ive_val) + kappa)
    else:
        # ive underflowed (small κ, large ν).
        # small-κ asymptotic:  I_ν(κ) ≈ (κ/2)^ν / Γ(ν+1)
...
def kappa_mle(r: float, d: int, *, n_newton: int = 3,
    ...
    """Banerjee approx + bounded Newton on A_d(κ) = r.

    Default cap kappa_max = 10*d. For n_c << d, the MLE is biased
    toward large κ; capping stabilizes log-likelihoods without
    materially affecting power for true high-concentration clusters
    (κ >> 10d gives essentially a Dirac in d=384; the cap costs at
    most a small power loss in the very-tight regime).
    """
    ...
    for _ in range(n_newton):
        ...
        # bounded step to avoid Newton overshoot at high d
...
# ---------- log q̂ via movMF mixture ----------

class QMovMF:
    """Cached movMF density evaluator."""
    def __init__(self, model: dict):
        self.mus = np.asarray(model["mus"], dtype=np.float32)            # (K, d)
        self.kappas = np.asarray(model["kappas"], dtype=np.float64)      # (K,)
        self.weights = np.asarray(model["weights"], dtype=np.float64)    # (K,)
        d = self.mus.shape[1]
        self.log_norms = np.array(
            [log_vmf_norm(d, k) for k in self.kappas]
        )                                                                 # (K,)
        self.log_w = np.log(self.weights + 1e-300)
        self.d = d

    def log_q(self, x: np.ndarray) -> np.ndarray:
        """log q̂(x_i)  for x: (n, d)  ->  (n,)."""
        inner = x @ self.mus.T                                            # (n, K)
        log_p = self.log_norms[None, :] + self.kappas[None, :] * inner    # (n, K)
        return logsumexp(log_p + self.log_w[None, :], axis=1)


# ---------- per-cluster e-value ----------

def split_lrt_log_e(x: np.ndarray, q: QMovMF, rng: np.random.Generator,
                   *, swap_and_average: bool = True,
                   ratio_log_cap: float = 50.0) -> tuple[float, float, float]:
    """Return (log_e, mu_hat_kappa, n_eff_for_e).

    Uses bounded log-ratio (cap at `ratio_log_cap` per observation) to control
    sup-norm misspecification per Theorem 2 / R6.
    """
    n = x.shape[0]
    ...
    # Vovk-Wang arithmetic mean of e-values: e_avg = (e1 + e2)/2
    # In log:  log e_avg = log[(exp log_e1 + exp log_e2)/2] = logsumexp - log 2
...
def main(*, embedding_path: Path | None = None,
         ...
         swap_and_average: bool = True,
         ratio_log_cap: float = 50.0,
         seed: int = 0,
         max_clusters: int | None = None) -> None:
    ...
    for i, (cid, group) in enumerate(groups):
        ...
        # guard against any zero-vector rows from whitening
        ...
        log_e, kappa_hat_avg, m_eff = split_lrt_log_e(
            x, q, rng,
            swap_and_average=swap_and_average,
            ratio_log_cap=ratio_log_cap,
        )
        # finite-cap on log_e for sane storage
        log_e = float(np.clip(log_e, -1e9, 1e9))
        e = float(np.exp(min(log_e, 700.0)))  # avoid overflow in float
        rows.append({
            "cluster_id": int(cid),
            "n": int(x.shape[0]),
            "m": int(m_eff),
            "kappa_hat": float(kappa_hat_avg),
            "log_e": log_e,
            "e": e,
        })
        if (i + 1) % print_every == 0:
            print(f"  {i + 1:,}/{n_clusters:,} clusters processed   "
                  f"({time() - t0:.1f}s)")
...
if __name__ == "__main__":
    ...
    p.add_argument("--no-swap", action="store_true",
                   help="single split (no swap-and-average); default is K=2 swap")
    p.add_argument("--ratio-log-cap", type=float, default=50.0,
                   help="per-observation log-ratio cap (Theorem 2 / R6 mitigation)")
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--max-clusters", type=int, default=None,
                   help="for testing: cap the number of clusters processed")
    args = p.parse_args()
