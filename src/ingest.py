...
SCHEMAS = {'submissions': (['submission_id', 'submission_type', 'express_comment', 'date_received', 'contact_email', 'city', 'address_line_1', 'address_line_2', 'state', 'zip_code', 'comment_id'], [pa.string()] * 2 + [pa.string(), pa.string()] + [pa.string()] * 7), 'comments': (['comment_id', 'comment_text', 'row_id'], [pa.string(), pa.string(), pa.int64()]), 'near_duplicates': (['target_document_id', 'duplicate_document_id'], [pa.string(), pa.string()]), 'exact_duplicates': (['target_document_id', 'duplicate_document_id'], [pa.string(), pa.string()]), 'interest_groups': (['submission_id', 'business'], [pa.string(), pa.string()]), 'filers': (['submission_id', 'filer_name'], [pa.string(), pa.string()]), 'documents': (['submission_id', 'document_name', 'download_status', 'document_id', 'file_extension'], [pa.string()] * 5), 'docs_cited': (['cite_id', 'submission_id', 'document_id'], [pa.string()] * 3)}
COPY_RE = re.compile('^COPY public\\.(\\w+) \\([^)]+\\) FROM stdin;\\s*$')
BATCH = 100000
_ESC = {'b': '\x08', 'f': '\x0c', 'n': '\n', 'r': '\r', 't': '\t', 'v': '\x0b', '\\': '\\'}
...
def write_batch(writer: pq.ParquetWriter, cols: list[str], types: list[pa.DataType], buf: list[list[str | None]]) -> None:
    arrays = []
...
def main() -> None:
    ...
    with RAW.open('r', encoding='utf-8', errors='replace') as f:
        ...
        while line:
            ...
            elif line.startswith('\\.') and (len(line) == 2 or line[2] in '\r\n'):
    ...
    for p in sorted(OUT.glob('*.parquet')):
        ...
        print(f'  {p.name:24s} {meta.num_rows:>12,} rows  {p.stat().st_size / 1000000.0:>7.1f} MB')
