"""Pre-flight checks before running run_v2.sh on Brev.

Verifies that all required artifacts from v1 are present and loadable, that
the new v2 code can read them, and that a tiny end-to-end smoke test of the
CPM + Beta-tail e-value pipeline works on a small subgraph.

Exits non-zero on any failure with a specific reason. Runs in ~30 seconds.
"""
...
REQUIRED_FROM_V1 = [
    PROC / "embeddings.npy",
    PROC / "embedding_index.parquet",
    PROC / "comments.parquet",
    PROC / "submissions.parquet",
]
...
def main() -> None:
    ...
    for p in REQUIRED_FROM_V1:
        check(p.name, p.exists(), f"size {p.stat().st_size/1e6:.1f} MB" if p.exists() else "missing")
    ...
    try:
        from cluster_singletons import main as _cs   # noqa: F401
        from evalues import fit_beta_mom, size_buckets, vovk_wang   # noqa: F401
        from resolution_sweep import main as _rs    # noqa: F401
        from cluster_eval import load_method        # noqa: F401
    ...
    check("Beta tail < 1e-3 at observed high T", sf_at_high < 1e-3,
          f"survival(0.5)={sf_at_high:.2e} (expect tiny since null mean ~0.2)")
