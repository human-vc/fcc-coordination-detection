"""Multi-resolution e-merge: aggregate cluster e-values across γ via Vovk-Wang.

For each cluster c that exists at multiple Leiden-CPM resolutions
γ ∈ {0.85, 0.88, 0.90, 0.93, 0.96}, we have multiple e-values (one per γ).
By Vovk-Wang 2021 Theorem 3.2, the arithmetic mean of e-values is itself an
e-value, valid under arbitrary dependence. Closure is the key property: we
can average across resolutions without needing independence between them.

Cluster matching across γ is by Jaccard overlap on member sets. For each
γ=0.90 cluster, we identify its "stable counterparts" at other γ — clusters
whose member sets overlap sufficiently. The averaged e-value is then a
robust statistic across resolution choice.

Validity: Σ E_c^bar ≤ Σ (1/K) Σ_γ E_c^γ ≤ K under nulls (compound e-value
property is preserved). e-BH on {E_c^bar} controls FDR.

Reference: Vovk & Wang (2021). E-values: calibration, combination,
...
def jaccard_match_clusters(target_cluster_id: int, target_members: set,
                           other_clusters: dict[int, set],
                           threshold: float = 0.5) -> int | None:
    """Find best Jaccard-overlap match in `other_clusters`."""
    best_cid = None
    best_jaccard = 0.0
...
def main() -> None:
    ...
    # For each γ, load cluster_id → log_e and cluster_id → set(member row_ids)
    g_data = {}
    for g in gammas:
        if g == "0.9":
        ...
        members = cl[cl["cluster_id"].isin(ev["cluster_id"])].groupby(
            "cluster_id")["row_id"].apply(set).to_dict()
        g_data[g] = {"ev": ev, "members": members}
        print(f"  γ={g}: {len(ev):,} clusters with LRT e-values, "
              f"{len(members):,} with member sets")

    # Anchor on γ=0.90 clusters; find Jaccard-matched counterparts at other γ
    target_g = "0.9"
    target_ev = g_data[target_g]["ev"]
    target_members = g_data[target_g]["members"]

    print()
    print("matching γ=0.90 clusters to counterparts at other γ "
    ...
    for _, r in target_ev.iterrows():
        ...
        # Vovk-Wang average:
        # avg(e_1, ..., e_J) is a valid e-value
        # log(avg) = logsumexp(log_es) - log(J)
        log_e_values = list(log_es.values())
        from scipy.special import logsumexp
        log_e_avg = float(logsumexp(log_e_values) - np.log(len(log_e_values)))
        rows.append({
            "cluster_id": cid_target,
            "n_resolutions_matched": len(log_es),
            "log_e_at_0.9": log_es[target_g],
            "log_e_avg": log_e_avg,
        })

    out = pd.DataFrame(rows)
    out.to_parquet(PROC / "cluster_evalues_lrt_multires.parquet",
    ...
    print(f"  single-γ (γ=0.90 only):  median={out['log_e_at_0.9'].median():.2f},  "
          f"p10={out['log_e_at_0.9'].quantile(0.1):.2f}")
    print(f"  multi-res averaged:       median={out['log_e_avg'].median():.2f},  "
          f"p10={out['log_e_avg'].quantile(0.1):.2f}")

    # e-BH on each
    def ebh(scores, alpha=0.10):
        K = len(scores)
        s = np.sort(scores)[::-1]
    ...
    def attribution(out, score_col, alpha=0.10):
        df = out.merge(att[["cluster_id", "frac_astroturf"]],
