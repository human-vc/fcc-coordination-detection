...
def main(*, batch_size: int | None=None, model_name: str='all-MiniLM-L6-v2', limit: int | None=None, output_path: Path | None=None) -> None:
    dev = device()
    ...
    idx[['row_id', 'comment_id', 'template_size']].to_parquet(IDX_PATH, compression='zstd', index=False)
    ...
    for i in tqdm(range(0, n, batch_size), desc='embed'):
        ...
        with torch.inference_mode():
            emb = model.encode(batch, batch_size=batch_size, convert_to_numpy=True, show_progress_bar=False, normalize_embeddings=True)
        out[i:i + len(batch)] = emb.astype(np.float16)
        written += len(batch)
    elapsed = time() - t0
    print(f'\nembedded {written:,} in {elapsed:.1f}s ({written / elapsed:.0f}/s)')
    np.save(out_path, out)
    print(f'wrote embeddings to {out_path}  ({out.nbytes / 1000000.0:.1f} MB)')
if __name__ == '__main__':
    p = argparse.ArgumentParser()
    p.add_argument('--batch-size', type=int, default=None, help='default: 1024 on CUDA, 256 on MPS, 64 on CPU')
    p.add_argument('--model', default='all-MiniLM-L6-v2', help='HF model name OR local path to fine-tuned model')
    p.add_argument('--output-path', type=Path, default=None, help='default: data/processed/embeddings.npy')
    p.add_argument('--limit', type=int, default=None, help='for testing: embed only first N rows')
    args = p.parse_args()
    main(batch_size=args.batch_size, model_name=args.model, limit=args.limit, output_path=args.output_path)
