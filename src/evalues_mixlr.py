...
RES = ROOT / 'results'
DEFAULT_EMB = PROC / 'embeddings.npy'
...
def main(*, embedding_path: Path | None=None, cluster_path: Path | None=None, tobs_path: Path | None=None, attr_path: Path | None=None, split_path: Path | None=None, output_path: Path | None=None, n_null_draws: int=2000, max_pairs: int=5000, n_buckets: int=30, seed: int=0) -> None:
    ...
    print(f'\nfitting g_0 (Beta MOM null) on {n_b} size buckets, {n_null_draws} draws each...')
    ...
    labeled = tobs.merge(att[['cluster_id', 'frac_astroturf', 'frac_advocacy', 'top_label']], on='cluster_id', how='left')
    ...
    print(f'\nlabeled clusters: astroturf>=0.5: {astroturf_mask.sum():,}, advocacy>=0.5: {advocacy_mask.sum():,}, either coord: {coord_mask.sum():,}')
    ...
    for (bid, s) in rep_size_per_bucket.items():
        ...
        g1_params[bid] = (a1, b1)
    print(f'  bucket-level coord counts: median {np.median(list(n_per_bucket.values())):.0f}, min {min(n_per_bucket.values())}, max {max(n_per_bucket.values())}')
    (pooled_a1, pooled_b1) = fit_beta_mom(coord_df['T_obs'].to_numpy())
    print(f'  pooled g_1 (Beta): a={pooled_a1:.2f}, b={pooled_b1:.2f}, mean={pooled_a1 / (pooled_a1 + pooled_b1):.4f}')
    (pooled_a0, pooled_b0) = fit_beta_mom(np.concatenate([np.array([cohesion(emb, rng.choice(b_rows, size=8, replace=False), max_pairs=max_pairs, rng=rng) for _ in range(2000)])]))
    print(f'  pooled g_0 (size~8): a={pooled_a0:.2f}, b={pooled_b0:.2f}, mean={pooled_a0 / (pooled_a0 + pooled_b0):.4f}')
    ...
    for (_, r) in tobs.iterrows():
        ...
        (a1, b1) = g1_params[bid]
        T = float(r['T_obs'])
        T_c = float(np.clip(T, 1e-06, 1.0 - 1e-06))
        ...
        rows.append({'cluster_id': int(r['cluster_id']), 'n': s, 'T_obs': T, 'log_g0': log_g0, 'log_g1': log_g1, 'log_e': log_e, 'e': e, 'bucket': bid})
    out = pd.DataFrame(rows)
    out.to_parquet(out_path, compression='zstd', index=False)
    print(f'\nwrote {out_path}  ({len(out):,} clusters)')
    print()
    out_lab = out.merge(att[['cluster_id', 'frac_astroturf', 'frac_advocacy', 'top_label']], on='cluster_id', how='left')
    grp_mask = {'all': out_lab['log_e'] == out_lab['log_e'], 'astroturf>=0.5': out_lab['frac_astroturf'].fillna(0.0) >= 0.5, 'advocacy>=0.5': out_lab['frac_advocacy'].fillna(0.0) >= 0.5, 'unlabeled': out_lab['top_label'].isna()}
    print('e-value distribution (log) by attribution group:')
    for (name, mask) in grp_mask.items():
        sub = out_lab[mask]
        if len(sub) == 0:
            continue
        print(f"  {name:18s}  n={len(sub):,}  log_e p10={sub['log_e'].quantile(0.1):6.2f}  p50={sub['log_e'].quantile(0.5):6.2f}  p90={sub['log_e'].quantile(0.9):6.2f}  p99={sub['log_e'].quantile(0.99):6.2f}")
if __name__ == '__main__':
    ...
    p.add_argument('--n-null-draws', type=int, default=2000)
    p.add_argument('--max-pairs', type=int, default=5000)
    p.add_argument('--n-buckets', type=int, default=30)
    p.add_argument('--seed', type=int, default=0)
    args = p.parse_args()
    main(embedding_path=args.embedding_path, cluster_path=args.cluster_path, tobs_path=args.tobs_path, attr_path=args.attr_path, split_path=args.split_path, output_path=args.output_path, n_null_draws=args.n_null_draws, max_pairs=args.max_pairs, n_buckets=args.n_buckets, seed=args.seed)
