...
def main(*, k: int=50, threshold: float=0.85, half: str='A') -> None:
    ...
    table = pa.table({'src_row': df['src'].to_numpy(dtype=np.int64), 'dst_row': df['dst'].to_numpy(dtype=np.int64), 'similarity': df['sim'].to_numpy(dtype=np.float32)})
    pq.write_table(table, EDGES_PATH, compression='zstd')
    print(f'\nwrote {EDGES_PATH}  ({EDGES_PATH.stat().st_size / 1000000.0:.1f} MB)')
if __name__ == '__main__':
    p = argparse.ArgumentParser()
    p.add_argument('--k', type=int, default=50)
    p.add_argument('--threshold', type=float, default=0.85)
    p.add_argument('--half', choices=['A', 'B', 'full'], default='A', help='which half of the sample-split to build the graph on')
    args = p.parse_args()
