...
from sklearn.metrics import average_precision_score, precision_score, recall_score, roc_auc_score, f1_score
from sklearn.model_selection import StratifiedKFold
ROOT = Path(__file__).resolve().parents[1]
PROC = ROOT / 'data' / 'processed'
RES = ROOT / 'results'

def assemble_features(att_path: Path=RES / 'attribution_table_r0.9.csv', lrt_path: Path=PROC / 'cluster_evalues_lrt_cluster_aware.parquet', mixlr_path: Path=PROC / 'cluster_evalues_mixlr_r0.9.parquet', cohesion_path: Path=PROC / 'cluster_evalues_leiden_r0.9.parquet') -> pd.DataFrame:
    ...
    df = lrt.merge(mixlr, on='cluster_id', how='inner').merge(coh, on='cluster_id', how='inner').merge(att[['cluster_id', 'frac_astroturf', 'frac_advocacy']], on='cluster_id', how='left')
    df['y'] = (df['frac_astroturf'].fillna(0) >= 0.5).astype(int)
    df['log_e_coh'] = np.log(df['e_coh'].astype(float).clip(1e-300, 1e+300))
...
def main() -> None:
    ...
    feature_cols = ['n', 'kappa_hat', 'log_e_lrt', 'log_e_mixlr', 'T_obs', 'T_obs_coh', 'p_coh', 'log_e_coh']
    ...
    print('-' * 75)
    for (name, Model) in [('logistic', LogisticRegression), ('gradient_boosted', GradientBoostingClassifier)]:
        (aucs, aps, ps, rs, f1s) = ([], [], [], [], [])
        for (tr, te) in skf.split(X, y):
            ...
            rs.append(best_r)
            f1s.append(best_f1)
        print(f'{name:<25}{np.mean(aucs):>8.3f}{np.mean(aps):>8.3f}{np.mean(ps):>11.3f}{np.mean(rs):>11.3f}{np.mean(f1s):>8.3f}')
    print()
    ...
    for (name, Model) in [('logistic', LogisticRegression), ('gradient_boosted', GradientBoostingClassifier)]:
        ...
        for target_recall in [1.0, 0.99, 0.95, 0.9]:
