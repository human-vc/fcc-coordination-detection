"""Graph-based coordination detection baseline.

Builds a kNN cosine-similarity graph on whitened embeddings, finds dense
subgraphs via Louvain modularity (community detection in CooRnet-style),
treats each community as a candidate "coordination cluster". Compare to
LRT pipeline.

Pacheco et al. and Magelinski-Carley (CooRnet) are the standard references
for graph-based coordinated-inauthentic-behavior detection. We approximate
their approach by:
  1. kNN graph on whitened SBERT embeddings (k=15)
  2. Modularity-based community detection (Leiden modularity, not CPM)
  3. Filter to communities of size ≥ 8
  4. Score each community by edge density / mean intra-community sim

This is a simpler proxy; the literature standard uses temporal coordination
signals + identity overlap which we don't have for FCC.

Reference:
  Pacheco et al. (2021). "Uncovering coordinated networks on social media"
    arXiv:2001.05658
  Magelinski, Ng, Carley (2022). "A synchronized action framework for
    detecting coordinated behavior". CooRnet R package.
"""
...
def main() -> None:
    ...
    # We need to attribute CC and HDBSCAN clusters separately since their
    # cluster_ids don't match Leiden's. Use member-overlap attribution.
    print()
    ...
    def per_cluster_attribution(cl_df, cu_df, min_size=8):
        ...
        df = df.merge(cu_df[["comment_id", "category"]],
        ...
        for cid, g in df.groupby("cluster_id"):
            ...
            rows.append({"cluster_id": int(cid), "n": n,
                         "frac_astroturf": n_astro / n if n else 0,
                         "frac_advocacy": n_adv / n if n else 0})
    ...
    print(f"{'method':<30}{'cands':>10}{'astro>=0.5':>14}{'astro%':>10}"
          f"{'adv>=0.5':>12}")
    print('-' * 75)
    ...
    summarize("Leiden-CPM γ=0.90 (ours)",
              att[att["n_members"] >= 8],
              len(att[att["n_members"] >= 8]))
