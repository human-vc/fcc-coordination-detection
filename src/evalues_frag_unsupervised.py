...
def fit_beta_2mix_em(f: np.ndarray, n_iter: int=100, tol: float=1e-05) -> tuple[dict, dict, float]:
    eps = 1e-06
    ...
    for it in range(n_iter):
        log_p0 = beta_dist.logpdf(f, a0, b0) + np.log(max(w0, 1e-09))
        log_p1 = beta_dist.logpdf(f, a1, b1) + np.log(max(w1, 1e-09))
        ...
        def fit(mu, var):
            ...
            if c <= 0:
                return (1.0, 1.0)
            return (max(mu * c, 0.001), max((1 - mu) * c, 0.001))
        (a0, b0) = fit(mu0, var0)
        (a1, b1) = fit(mu1, var1)
        w0 = n0 / (n0 + n1)
        w1 = 1 - w0
    ...
    return ({'a': a0, 'b': b0, 'mean': a0 / (a0 + b0), 'weight': w0}, {'a': a1, 'b': b1, 'mean': a1 / (a1 + b1), 'weight': w1}, float(log_lik_prev))

def closed_ebh(scores: np.ndarray, alpha: float=0.1) -> int:
...
def vanilla_ebh(scores: np.ndarray, alpha: float=0.1) -> int:
    e = np.exp(np.clip(scores, -700, 700))
    K = len(e)
...
def main(*, alpha: float=0.1) -> None:
    ...
    print(f"  g_0 (verbatim, low-f):    Beta({g0['a']:.2f}, {g0['b']:.2f}), mean = {g0['mean']:.3f}, weight = {g0['weight']:.3f}")
    print(f"  g_1 (paraphrase, high-f): Beta({g1['a']:.2f}, {g1['b']:.2f}), mean = {g1['mean']:.3f}, weight = {g1['weight']:.3f}")
    ...
    print(f"  unsupervised match: {abs(g0['mean'] - 0.467):.3f} & {abs(g1['mean'] - 0.881):.3f} away")
    f_clipped = np.clip(f, 1e-06, 1 - 1e-06)
    ...
    print(f"  closed-eBH:      rejects {k_cls:,}  ({('gain' if k_cls > k_van else 'tie')} of {k_cls - k_van:+,})")
    ...
    print('-' * 70)
    for (name, rej) in [('vanilla e-BH', rej_van), ('closed-eBH', rej_cls)]:
        n = len(rej)
        n_astro = int(rej['y_astro'].sum())
        n_adv = int(rej['y_adv'].sum())
        rec = n_astro / max(int(frag['y_astro'].sum()), 1)
        print(f'{name:<22}{n:>8,}{n_astro:>8,}{100 * n_astro / max(n, 1):>9.1f}%{n_adv:>6,}{100 * rec:>13.1f}%')
    ...
    out = frag.copy()
    out.to_parquet(PROC / 'cluster_evalues_fragmentation_unsup.parquet', compression='zstd', index=False)
