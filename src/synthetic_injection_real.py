"""Synthetic-injection power figure with real-corpus geometry.

For each (cluster size n, concentration κ): generate synthetic vMF clusters
seeded at random real-corpus directions, embed them as if real, run the
full LRT pipeline, measure rejection rate. Plot empirical recovery vs the
theoretical κ ≥ C·d/√m bound.

This validates Theorem 3 in the actual SBERT geometry rather than pure
isotropic vMF.
"""
...
def power_at(d, n, kappa, q, real_seeds, n_trials, rng):
    ...
    for t in range(n_trials):
        # Pick a random real-corpus embedding as seed direction
...
def main(*, n_seeds: int = 5000, n_trials: int = 80) -> None:
    ...
    # Sample seeds from singletons (cluster-disjoint)
    ...
    for n in ns:
        ...
        for kappa in kappas:
            ...
            line += f"  {p:>5.2f}"
            rows.append({"n": n, "kappa": kappa, "power": p,
                         "theory_threshold_C1.4": 1.4 * d / math.sqrt(n / 2),
                         "theory_threshold_C2.4": 2.4 * d / math.sqrt(n / 2)})
        print(line)
    ...
    try:
        ...
        for i, n in enumerate(ns):
            ...
            ax.plot(sub["kappa"], sub["power"], marker="o",
                    color=colors[i], label=f"n={n}", linewidth=2)
            # vertical line at theoretical kappa* (C=1.4 * d/sqrt(n/2))
            ax.axvline(1.4 * d / math.sqrt(n / 2), color=colors[i],
                       ls=":", alpha=0.4)
        ax.set_xscale("log")
        ax.set_xlabel(r"injected $\kappa$", fontsize=11)
        ax.set_ylabel("empirical rejection rate (real-corpus seeds)",
                      fontsize=11)
        ax.set_title(r"Synthetic injection in real SBERT geometry: "
                     r"vertical lines = theoretical $\kappa^*$ at $C=1.4$",
                     fontsize=10)
