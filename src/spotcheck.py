"""Print sample comment texts from rejected clusters for human spot-checking.

Stratified sampling: pick clusters from small/medium/large size buckets, then
within each cluster, sample comments stratified by template_size so we see
both heavy-template anchors and singleton members (the soft-coordination
candidates).

Outputs results/spotcheck_<method>.txt — readable in a terminal or editor.
"""
...
def pick_clusters(rj: pd.DataFrame, cl: pd.DataFrame, *,
                  buckets: list[tuple[int, int]],
                  per_bucket: int, seed: int) -> list[tuple[str, int]]:
...
def get_text_lookup() -> dict[str, str]:
    ...
    df = pq.read_table(PROC / "comments.parquet",
                       columns=["comment_id", "comment_text"]).to_pandas()
    return dict(zip(df["comment_id"].astype(str),
                    df["comment_text"].fillna("").astype(str)))


def render_cluster(cid: int, cl: pd.DataFrame, text_lookup: dict[str, str],
                   per_template_strata: int = 2) -> str:
    members = cl[cl["cluster_id"] == cid]
    n = len(members)
    out = [f"\n{'=' * 76}",
           f"cluster_id={cid}  n_members={n}"]

    # stratified sample by template_size class
    strata = [
        ("singletons", members[members["template_size"] == 1]),
        ("small (2-10)", members[members["template_size"].between(2, 10)]),
        ("medium (11-100)", members[members["template_size"].between(11, 100)]),
        ("large (101+)", members[members["template_size"] >= 101]),
    ]
    ...
    for name, sub in strata:
        ...
        for _, row in sample.iterrows():
            ...
            out.append(f"    [tmpl={row['template_size']:>7,}] "
                       f"{preview!r}")
    return "\n".join(out)
...
def main(*, methods: list[str], per_bucket: int, seed: int) -> None:
    ...
    buckets = [(5, 20), (20, 100), (100, 1000), (1000, 1_000_000)]
    ...
    for method in methods:
        try:
            ...
            continue
        picked = pick_clusters(rj, cl, buckets=buckets,
                               per_bucket=per_bucket, seed=seed)
        if not picked:
            continue

        out_path = RES / f"spotcheck_{method}.txt"
        lines = [f"# spot-check for method={method}",
                 f"# {len(picked)} clusters across buckets "
                 f"{[(lo,hi) for lo,hi in buckets]}, "
                 f"{per_bucket} per bucket"]
