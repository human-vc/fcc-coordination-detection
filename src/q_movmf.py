...
def kappa_mle(r: float, d: int, n_newton: int=3) -> float:
    ...
    for _ in range(n_newton):
        ...
        kappa = max(kappa - (A - r) / max(A_prime, 1e-09), 0.001)
    return float(kappa)
...
def _log_responsibilities(x: np.ndarray, mus: np.ndarray, kappas: np.ndarray, weights: np.ndarray) -> tuple[np.ndarray, float]:
    d = x.shape[1]
...
def _initialize(x: np.ndarray, K: int, rng: np.random.Generator, init_kappa: float=50.0) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    (n, d) = x.shape
...
def fit_movmf(x: np.ndarray, K: int, *, n_iter: int=50, tol: float=0.001, rng: np.random.Generator | None=None, verbose: bool=True) -> dict:
    ...
    for it in range(n_iter):
        ...
        if abs(ll - prev_ll) < tol * abs(prev_ll):
            ...
            break
        prev_ll = ll
    return {'mus': mus, 'kappas': kappas, 'weights': weights, 'log_likelihood_train': float(ll), 'K': K, 'n_iter_done': it + 1}

def heldout_loglik(x_held: np.ndarray, mus: np.ndarray, kappas: np.ndarray, weights: np.ndarray) -> float:
    (_, ll) = _log_responsibilities(x_held, mus, kappas, weights)
    return float(ll / x_held.shape[0])

def log_q_movmf(x: np.ndarray, mus: np.ndarray, kappas: np.ndarray, weights: np.ndarray) -> np.ndarray:
    d = x.shape[1]
    log_norms = np.array([log_vmf_norm(d, k) for k in kappas])
    inner = x @ mus.T
    log_p = log_norms[None, :] + kappas[None, :] * inner
...
def add_tail_pad(model: dict, *, pad_weight: float=0.01, pad_kappa: float=1.0, d: int | None=None, rng: np.random.Generator | None=None) -> dict:
    rng = rng or np.random.default_rng(0)
    mus = model['mus']
    if d is None:
        d = mus.shape[1]
    ...
    new_weights = np.concatenate([model['weights'] * (1 - pad_weight), [pad_weight]])
    out = dict(model)
    out['mus'] = new_mus
...
def main(*, input_path: Path | None=None, output_path: Path | None=None, n_train: int=200000, n_held: int=50000, Ks: tuple[int, ...]=(5, 10, 25, 50), n_iter: int=50, seed: int=0, pad_weight: float=0.01, pad_kappa: float=1.0) -> None:
    ...
    for K in Ks:
        ...
        elapsed = time() - t0
        print(f"  K={K:3d}  ll_train/n={model['log_likelihood_train'] / n_train:.4f}  ll_held/n={ll_held:.4f}  ({elapsed:.1f}s)")
        results.append({'K': K, 'model': model, 'll_held_per_obs': ll_held, 'elapsed_s': elapsed})
    best = max(results, key=lambda r: r['ll_held_per_obs'])
    print(f"\nbest K_q = {best['K']}  (held-out ll/n = {best['ll_held_per_obs']:.4f})")
    final = add_tail_pad(best['model'], pad_weight=pad_weight, pad_kappa=pad_kappa, d=d, rng=rng)
    final['log_likelihood_heldout'] = best['ll_held_per_obs']
    final['K_sweep_results'] = [{'K': r['K'], 'll_held_per_obs': r['ll_held_per_obs']} for r in results]
    final['dim'] = d
...
if __name__ == '__main__':
    ...
    p.add_argument('--input-path', type=Path, default=None, help='default: data/processed/embeddings_white_k5.npy')
    p.add_argument('--output-path', type=Path, default=None, help='default: data/processed/q_movmf.pkl')
    p.add_argument('--n-train', type=int, default=200000)
    p.add_argument('--n-held', type=int, default=50000)
    ...
    args = p.parse_args()
    main(input_path=args.input_path, output_path=args.output_path, n_train=args.n_train, n_held=args.n_held, Ks=tuple(args.Ks), n_iter=args.n_iter, pad_weight=args.pad_weight, pad_kappa=args.pad_kappa, seed=args.seed)
