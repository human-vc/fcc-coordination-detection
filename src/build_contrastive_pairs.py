...
def main(*, n_positive_per_uploader: int=200, min_uploader_count: int=100, max_uploaders: int=200, seed: int=0) -> None:
    ...
    print(f'\nwrote {OUT_PATH}  ({OUT_PATH.stat().st_size / 1000000.0:.1f} MB)')
if __name__ == '__main__':
    p = argparse.ArgumentParser()
    p.add_argument('--n-positive-per-uploader', type=int, default=200, help='how many positive pairs to draw per uploader')
    p.add_argument('--min-uploader-count', type=int, default=100, help='ignore uploaders with fewer than this many comments')
    p.add_argument('--max-uploaders', type=int, default=200, help='cap to top-N uploaders by volume')
    p.add_argument('--seed', type=int, default=0)
    args = p.parse_args()
    main(n_positive_per_uploader=args.n_positive_per_uploader, min_uploader_count=args.min_uploader_count, max_uploaders=args.max_uploaders, seed=args.seed)
