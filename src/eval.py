...
def confusion(y_true: np.ndarray, y_pred: np.ndarray) -> dict[str, int]:
    return {'TP': int((y_true & y_pred).sum()), 'FP': int((~y_true & y_pred).sum()), 'FN': int((y_true & ~y_pred).sum()), 'TN': int((~y_true & ~y_pred).sum())}
...
def evaluate_method(method: str, *, gold_thresholds: list[int], spot_check_n: int, rng: np.random.Generator) -> list[dict]:
    ...
    for gt in gold_thresholds:
        ...
        m = metrics(c)
        rows.append({'method': method, 'gold_threshold': gt, **c, **m})
    sing = cl[cl['template_size'] == 1]
    n_sing = len(sing)
    n_sing_pred = int(sing['pred'].sum())
    rows.append({'method': method, 'gold_threshold': -1, 'TP': -1, 'FP': -1, 'FN': -1, 'TN': -1, 'precision': float('nan'), 'recall': float('nan'), 'f1': float('nan'), 'singletons_total': n_sing, 'singletons_predicted_coord': n_sing_pred})
    sing_clusters = sing.loc[sing['pred']].groupby('cluster_id').size().sort_values(ascending=False)
    if len(sing_clusters):
        ...
        for (cid, sz) in pick.items():
            ...
            spot_rows.append({'cluster_id': int(cid), 'size': int(sz), 'row_ids_sample': mems})
        pd.DataFrame(spot_rows).to_csv(spot_path, index=False)
    return rows

def main(*, methods: list[str], gold_thresholds: list[int], spot_check_n: int, seed: int) -> None:
    rng = np.random.default_rng(seed)
    all_rows = []
    for method in methods:
        try:
            all_rows.extend(evaluate_method(method, gold_thresholds=gold_thresholds, spot_check_n=spot_check_n, rng=rng))
...
if __name__ == '__main__':
    ...
    p.add_argument('--gold-thresholds', nargs='+', type=int, default=[10, 100, 1000, 10000])
    p.add_argument('--spot-check-n', type=int, default=20)
    p.add_argument('--seed', type=int, default=0)
    args = p.parse_args()
    main(methods=args.methods, gold_thresholds=args.gold_thresholds, spot_check_n=args.spot_check_n, seed=args.seed)
