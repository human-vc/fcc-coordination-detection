...
def write_clusters(idx: pd.DataFrame, cluster_for_row: np.ndarray, name: str, min_cluster_size: int) -> None:
    out = idx.copy()
    ...
    print(f'  {name}: {len(big):,} clusters with size >= {min_cluster_size}; largest = {(int(big.iloc[0]) if len(big) else 0)}')
...
def run_minhash_lsh(idx: pd.DataFrame, a_mask: np.ndarray, *, threshold: float=0.85, num_perm: int=128, shingle_n: int=5, min_cluster_size: int=5, subsample: int | None=None) -> None:
...
def run_connected_components(idx: pd.DataFrame, a_mask: np.ndarray, *, min_cluster_size: int=5) -> None:
...
def run_hdbscan(idx: pd.DataFrame, a_mask: np.ndarray, *, min_samples: int=5, min_cluster_size: int=5, subsample: int=200000) -> None:
    import hdbscan
    ...
    if len(a_rows) > subsample:
        ...
        print(f'[hdbscan_emb] HDBSCAN does not scale to 1.9M; subsampling to {subsample:,} A-rows (random seed=0)')
    else:
        a_rows_sub = a_rows
    sub = emb[a_rows_sub]
    print(f'[hdbscan_emb] running HDBSCAN on {len(sub):,} vectors...')
    t0 = time()
    clusterer = hdbscan.HDBSCAN(metric='euclidean', min_cluster_size=min_cluster_size, min_samples=min_samples, core_dist_n_jobs=-1, algorithm='boruvka_kdtree')
    labels = clusterer.fit_predict(sub)
    print(f'  HDBSCAN done in {time() - t0:.1f}s')
    cluster_for_row = np.full(len(idx), -1, dtype=np.int64)
    cluster_for_row[a_rows_sub] = labels
    cluster_for_row[~a_mask] = -2
    write_clusters(idx, cluster_for_row, 'hdbscan_emb', min_cluster_size)

def main(*, methods: list[str], threshold: float, min_cluster_size: int, minhash_subsample: int | None=500000, minhash_num_perm: int=64, hdbscan_subsample: int=200000) -> None:
    (idx, a_mask) = load_split()
    if 'minhash_lsh' in methods:
        run_minhash_lsh(idx, a_mask, threshold=threshold, num_perm=minhash_num_perm, min_cluster_size=min_cluster_size, subsample=minhash_subsample)
    ...
    if 'hdbscan_emb' in methods:
        run_hdbscan(idx, a_mask, min_cluster_size=min_cluster_size, subsample=hdbscan_subsample)
if __name__ == '__main__':
    p = argparse.ArgumentParser()
    p.add_argument('--methods', nargs='+', default=['minhash_lsh', 'connected_components', 'hdbscan_emb'], choices=['minhash_lsh', 'connected_components', 'hdbscan_emb'])
    p.add_argument('--threshold', type=float, default=0.85, help='MinHash Jaccard threshold (also used for kNN)')
    p.add_argument('--min-cluster-size', type=int, default=5)
    p.add_argument('--minhash-subsample', type=int, default=500000, help='cap MinHash to this many A-rows (OOM at 1.9M)')
    p.add_argument('--minhash-num-perm', type=int, default=64, help='MinHash permutations (lower = less memory)')
    p.add_argument('--hdbscan-subsample', type=int, default=200000, help="HDBSCAN doesn't scale to >~300K; subsample first")
    args = p.parse_args()
    main(methods=args.methods, threshold=args.threshold, min_cluster_size=args.min_cluster_size, minhash_subsample=args.minhash_subsample, minhash_num_perm=args.minhash_num_perm, hdbscan_subsample=args.hdbscan_subsample)
