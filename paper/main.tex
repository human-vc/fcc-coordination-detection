...
\begin{abstract}
Coordinated paraphrase campaigns in regulatory public-comment dockets are paraphrased templates: paid contractors deliberately introduce variation to evade simple text-match detection. Standard concentration-based detectors (vMF likelihood, mean-cohesion) are anti-correlated with paid-astroturf attribution on real corpora because advocacy form letters use \emph{verbatim} templates while paid astroturf \emph{paraphrases}. We show that paraphrase variation is a detectable signature in multi-resolution clustering: a paraphrase-coordinated cluster fragments into many sub-clusters as the clustering resolution tightens, while a verbatim-coordinated cluster stays together. We propose a compound e-value with the cluster-level fragmentation rate $f_c = (\text{distinct sub-clusters at } \gamma{=}0.96 \text{ among } c\text{'s members}) / |c|$ as test statistic, and an unsupervised Beta-mixture EM calibration of the verbatim ($g_0$) vs paraphrase ($g_1$) components. The construction is finite-sample valid by the Ignatiadis--Wang--Ramdas \citeyearpar{iwr2024} \S 7 likelihood-ratio identity; aggregating with e-BH \citep{wangramdas2022} gives FDR control at level $\alpha$. We separately develop a vMF-template compound e-value via universal inference \citep{wrb2020} with explicit power threshold $\kappa^\star \le C \cdot d/\sqrt{m}$, $C \le 2.4$ (median $1.40$), validated by synthetic injection at real-corpus seeds. Applied at $\alpha = 0.10$ to the FCC 2017 ``Restoring Internet Freedom'' docket on 15{,}748 size-$\ge$-8 Leiden-CPM clusters, the fragmentation procedure rejects 2{,}595 clusters at 84.7\% precision and 36.2\% recall against paid-astroturf clusters identified by the 2021 NY Attorney General investigation, \emph{with no labels used}. Standard concentration-based detectors flag all 15{,}748 candidates non-selectively at the same $\alpha$. The fragmentation e-value AP against NYAG attribution is 0.815, vs base rate 0.386; a label-trained supervised classifier reaches AP 0.946 oracle. Multi-resolution fragmentation operationalizes the paraphrase-vs-verbatim distinction the corpus's coordination types exhibit.
\end{abstract}

\section{Introduction}

Federal regulators received over 22 million public comments on the FCC's 2017 ``Restoring Internet Freedom'' docket alone, and similar volumes on the CFPB payday lending rule, EPA Clean Power Plan repeal, and SEC climate disclosure rule. Investigative journalism and government investigations have established that a large fraction of these comments are coordinated: identical or near-identical text submitted under thousands of different names, sometimes with identities stolen from public records. The 2021 New York Attorney General report \citep{nyag2021} attributed 18 million FCC comments to specific paid contractors who fabricated submissions on behalf of broadband industry clients; the \emph{Wall Street Journal} \citeyearpar{wsj2017} found that 40\% of named CFPB-2016-0025 commenters whom they phoned denied authoring the comments in their name. These campaigns directly shape final agency rulemaking.
...
\paragraph{E-values and FDR.} E-values \citep{vovk2021,gw2021} provide a direct alternative to p-values for hypothesis testing under arbitrary dependence. The e-BH procedure of \citet{wangramdas2022} controls FDR at level $\alpha$ given any compound family of e-values, regardless of dependence. Universal inference \citep{wrb2020} produces likelihood-ratio e-values without regularity conditions; the construction we use specializes their split-LRT to a single-density null. The IWR \citeyearpar{iwr2024} \S 7 mixture-LR identity gives compound e-values from likelihood ratios of fitted alternatives. Boosting via conditional calibration \citep{leeren2024} and closed-eBH \citep{xfr2025} provide strict improvements over vanilla e-BH on individually-valid e-values; we comment on closed-eBH's behavior on heavy-tailed e-value distributions in \S\ref{sec:experiments}.

\paragraph{Coordination detection in NLP.} Most prior work on duplicate-comment detection in regulatory dockets uses exact-match or near-duplicate hashing \citep{kao2018,handan2022}, which catches verbatim form letters but misses paraphrase coordination. \citet{handan2022} provided the canonical FCC 17-108 corpus annotated with exact and near-duplicate identifiers, used as our embedding source. The NYU SMaPP and Stanford NetSec teams have produced supervised coordination classifiers \citep{neudert2017} but require gold labels that are scarce in this setting. Our fragmentation construction is unsupervised on the candidate set.
...
We apply both constructions to the FCC 17-108 ``Restoring Internet Freedom'' docket of \citet{handan2022}, comprising 22 million comments. We extract unique comment texts (post deduplication), embed via SBERT MiniLM-L6-v2, apply all-but-the-top whitening with $k = 5$ \citep{mu2018}, and cluster via Leiden CPM at $\gamma = 0.90$. This yields 21{,}606 candidate clusters; restricting to size $\ge 8$ for the vMF e-value gives $K = 15{,}748$.
